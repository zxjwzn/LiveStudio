...
class ModelPosition(VTSBaseModel):
    ...
    position_x: float = Field(alias="positionX", description="模型 X 坐标，通常范围为 -1000 到 1000。")
    position_y: float = Field(alias="positionY", description="模型 Y 坐标，通常范围为 -1000 到 1000。")
...
class AvailableModel(VTSBaseModel):
    ...
    model_loaded: bool = Field(alias="modelLoaded", description="该模型当前是否已加载。")
    model_name: str = Field(alias="modelName", description="模型显示名称。")
    model_id: str = Field(alias="modelID", description="模型唯一标识。")
    vts_model_name: str = Field(alias="vtsModelName", description="VTS 模型配置文件名。")
...
class Hotkey(VTSBaseModel):
    ...
    key_combination: list[str] = Field(alias="keyCombination", description="按键组合列表；当前通常为空。")
    on_screen_button_id: int = Field(alias="onScreenButtonID", description="屏幕按钮 ID，-1 表示未设置。")
...
class ExpressionState(VTSBaseModel):
    ...
    deactivate_when_key_is_let_go: bool = Field(alias="deactivateWhenKeyIsLetGo", description="按键松开时是否自动取消。")
    auto_deactivate_after_seconds: bool = Field(alias="autoDeactivateAfterSeconds", description="是否启用定时自动取消。")
    seconds_remaining: float = Field(alias="secondsRemaining", description="剩余自动取消秒数。")
    used_in_hotkeys: list[ExpressionHotkeyRef] = Field(alias="usedInHotkeys", description="引用该表达式的热键列表。")
    parameters: list[ExpressionParameter] = Field(description="表达式中涉及的参数与目标值。")
...
class ParameterInfo(VTSBaseModel):
    ...
    added_by: str | None = Field(default=None, alias="addedBy", description="参数创建者。默认参数通常为 `VTube Studio`。")
...
class PhysicsGroup(VTSBaseModel):
    ...
    strength_multiplier: float = Field(alias="strengthMultiplier", description="物理强度倍率。")
...
class PhysicsOverride(VTSBaseModel):
    ...
    set_base_value: bool = Field(alias="setBaseValue", description="是否覆盖基础值而非特定分组。")
    override_seconds: float = Field(alias="overrideSeconds", ge=0.5, le=5.0, description="覆盖持续秒数。")
...
class ColorTint(VTSBaseModel):
    ...
    mix_with_scene_lighting_color: float | None = Field(default=None, alias="mixWithSceneLightingColor", ge=0, le=1, description="与场景光照颜色混合比。")


class ArtMeshMatcher(VTSBaseModel):
    """ArtMesh 匹配条件。"""

    tint_all: bool = Field(default=False, alias="tintAll", description="是否对全模型应用着色。")
    art_mesh_number: list[int] = Field(default_factory=list, alias="artMeshNumber", description="按序号匹配 ArtMesh。")
    name_exact: list[str] = Field(default_factory=list, alias="nameExact", description="按名称精确匹配。")
    name_contains: list[str] = Field(default_factory=list, alias="nameContains", description="按名称包含匹配。")
    tag_exact: list[str] = Field(default_factory=list, alias="tagExact", description="按标签精确匹配。")
    tag_contains: list[str] = Field(default_factory=list, alias="tagContains", description="按标签包含匹配。")
...
class InjectParameterValue(VTSBaseModel):
    ...
    weight: float | None = Field(default=None, ge=0, le=1, description="混合权重；`set` 模式可选。")
...
class ItemInstance(VTSBaseModel):
    ...
    frame_count: int = Field(alias="frameCount", description="总帧数，非动画通常为 -1。")
    ...
    pinned_art_mesh_id: str = Field(alias="pinnedArtMeshID", description="固定到的 ArtMesh ID。")
...
class ItemMove(VTSBaseModel):
    ...
    time_in_seconds: float = Field(alias="timeInSeconds", ge=0, le=30, description="过渡耗时秒数。")
    fade_mode: FadeMode = Field(alias="fadeMode", description="移动插值模式。")
    position_x: float = Field(alias="positionX", description="目标 X 坐标，<= -1000 表示忽略。")
    position_y: float = Field(alias="positionY", description="目标 Y 坐标，<= -1000 表示忽略。")
    ...
    user_can_stop: bool = Field(alias="userCanStop", description="用户拖拽时是否可中断移动。")
...
class PinInfo(VTSBaseModel):
    ...
    model_id: str = Field(alias="modelID", description="目标模型 ID；为空时使用当前模型。")
    art_mesh_id: str = Field(alias="artMeshID", description="目标 ArtMesh ID；为空时随机。")
...
class PostProcessingValue(VTSBaseModel):
    ...
    config_value: str = Field(alias="configValue", description="配置项值，始终以字符串形式发送。")
...
class PostProcessingConfigEntry(VTSBaseModel):
    ...
    activation_config: bool = Field(alias="activationConfig", description="是否为激活效果的关键配置。")
    ...
    color_has_alpha: bool = Field(alias="colorHasAlpha", description="颜色配置是否支持透明度。")
    ...
    scene_item_default: str = Field(alias="sceneItemDefault", description="默认场景道具值。")
...
class PostProcessingEffect(VTSBaseModel):
    ...
    effect_is_active: bool = Field(alias="effectIsActive", description="效果是否正在生效。")
    effect_is_restricted: bool = Field(alias="effectIsRestricted", description="是否为受限/实验性效果。")
    config_entries: list[PostProcessingConfigEntry] = Field(alias="configEntries", description="该效果的全部配置项。")
