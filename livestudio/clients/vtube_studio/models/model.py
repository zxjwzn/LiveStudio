...
class CurrentModelRequest(VTSRequestEnvelope[VTSEmptyData]):
    ...
    message_type: str = Field(
        default="CurrentModelRequest",
        alias="messageType",
        description="请求当前模型详细信息",
    )
...
class CurrentModelResponseData(VTSEmptyData):
    ...
    live2d_model_name: str = Field(
        alias="live2DModelName",
        description="Live2D 模型 JSON 文件名",
    )
    model_load_time: int = Field(
        alias="modelLoadTime",
        description="本次模型加载耗时，毫秒",
    )
    time_since_model_loaded: int = Field(
        alias="timeSinceModelLoaded",
        description="距离模型加载完成的毫秒数",
    )
    number_of_live2d_parameters: int = Field(
        alias="numberOfLive2DParameters",
        description="Live2D 参数数量",
    )
    number_of_live2d_artmeshes: int = Field(
        alias="numberOfLive2DArtmeshes",
        description="ArtMesh 数量",
    )
    has_physics_file: bool = Field(
        alias="hasPhysicsFile",
        description="是否存在物理文件",
    )
    number_of_textures: int = Field(alias="numberOfTextures", description="纹理数量")
    texture_resolution: int = Field(
        alias="textureResolution",
        description="纹理分辨率",
    )
    model_position: ModelPosition = Field(
        alias="modelPosition",
        description="模型位置、旋转、大小",
    )
...
class AvailableModelsRequest(VTSRequestEnvelope[VTSEmptyData]):
    ...
    message_type: str = Field(
        default="AvailableModelsRequest",
        alias="messageType",
        description="请求本地全部可用模型",
    )
    data: VTSEmptyData = Field(
        default_factory=VTSEmptyData,
        description="该请求无需额外参数",
...
class ModelLoadRequestData(VTSEmptyData):
    ...
    model_id: str = Field(
        alias="modelID",
        description="目标模型 ID；空字符串表示卸载当前模型",
    )
...
class ModelLoadRequest(VTSRequestEnvelope[ModelLoadRequestData]):
    ...
    message_type: str = Field(
        default="ModelLoadRequest",
        alias="messageType",
        description="请求加载指定模型",
    )
...
class MoveModelRequestData(VTSEmptyData):
    ...
    time_in_seconds: float = Field(
        ...
        ge=0,
        le=2,
        description="移动过渡要多久，单位秒，0 表示瞬间移动",
    )
    values_are_relative_to_model: bool = Field(
        alias="valuesAreRelativeToModel",
        description="坐标、旋转、尺寸是否相对当前模型状态",
    )
    ...
    position_y: float | None = Field(
        ...
        ge=-1000,
        le=1000,
        description="可选目标 Y 坐标",
    )
    rotation: float | None = Field(
        default=None,
        ge=-360,
        le=360,
        description="可选目标旋转角度",
    )
...
class MoveModelRequest(VTSRequestEnvelope[MoveModelRequestData]):
    ...
    message_type: str = Field(
        default="MoveModelRequest",
        alias="messageType",
        description="请求移动当前已加载模型",
    )
