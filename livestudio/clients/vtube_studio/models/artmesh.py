...
class ArtMeshListRequest(VTSRequestEnvelope[VTSEmptyData]):
    ...
    message_type: str = Field(
        default="ArtMeshListRequest",
        alias="messageType",
        description="请求当前模型所有 ArtMesh 名称与标签。",
    )
...
class ArtMeshListResponseData(VTSEmptyData):
    ...
    number_of_art_mesh_names: int = Field(
        alias="numberOfArtMeshNames",
        description="ArtMesh 名称数量。",
    )
    number_of_art_mesh_tags: int = Field(
        alias="numberOfArtMeshTags",
        description="ArtMesh 标签数量。",
    )
    art_mesh_names: list[str] = Field(
        alias="artMeshNames",
        description="ArtMesh ID 列表。",
    )
    art_mesh_tags: list[str] = Field(
        alias="artMeshTags",
        description="全部唯一标签列表。",
    )
...
class ColorTintRequestData(VTSEmptyData):
    ...
    art_mesh_matcher: ArtMeshMatcher = Field(
        alias="artMeshMatcher",
        description="匹配 ArtMesh 的筛选规则。",
    )
...
class ColorTintRequest(VTSRequestEnvelope[ColorTintRequestData]):
    ...
    message_type: str = Field(
        default="ColorTintRequest",
        alias="messageType",
        description="请求为匹配的 ArtMesh 应用颜色。",
    )
...
class ColorTintResponseData(VTSEmptyData):
    ...
    matched_art_meshes: int = Field(
        alias="matchedArtMeshes",
        description="本次匹配并应用的 ArtMesh 数量。",
    )
...
class SceneColorOverlayInfoRequest(VTSRequestEnvelope[VTSEmptyData]):
    ...
    message_type: str = Field(
        default="SceneColorOverlayInfoRequest",
        alias="messageType",
        description="请求当前场景光照叠加配置。",
    )
    data: VTSEmptyData = Field(
        default_factory=VTSEmptyData,
        description="该请求无需额外参数。",
...
class SceneColorOverlayInfoResponseData(VTSEmptyData):
    ...
    items_included: bool = Field(
        alias="itemsIncluded",
        description="是否同时影响道具。",
    )
    is_window_capture: bool = Field(
        alias="isWindowCapture",
        description="是否为窗口采样。",
    )
    ...
    left_capture_part: CapturePart = Field(
        alias="leftCapturePart",
        description="左侧采样区域。",
    )
    middle_capture_part: CapturePart = Field(
        alias="middleCapturePart",
        description="中间采样区域。",
    )
    right_capture_part: CapturePart = Field(
        alias="rightCapturePart",
        description="右侧采样区域。",
    )


class SceneColorOverlayInfoResponse(
    VTSResponseEnvelope[SceneColorOverlayInfoResponseData],
):
    """场景光照叠加响应信封。"""
