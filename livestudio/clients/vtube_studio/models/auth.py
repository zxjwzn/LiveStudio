...
class APIStateRequest(VTSRequestEnvelope[VTSEmptyData]):
    ...
    message_type: str = Field(
        default="APIStateRequest",
        alias="messageType",
        description="请求当前 API 状态。",
    )
...
class APIStateResponseData(VTSEmptyData):
    ...
    current_session_authenticated: bool = Field(
        alias="currentSessionAuthenticated",
        description="当前 WebSocket 会话是否已认证。",
    )
...
class AuthenticationTokenRequestData(VTSEmptyData):
    ...
    plugin_icon: str | None = Field(
        default=None,
        alias="pluginIcon",
        description="可选 128x128 Base64 图标。",
    )
...
class AuthenticationTokenRequest(VTSRequestEnvelope[AuthenticationTokenRequestData]):
    ...
    message_type: str = Field(
        default="AuthenticationTokenRequest",
        alias="messageType",
        description="向用户申请令牌授权。",
    )
...
class AuthenticationRequestData(VTSEmptyData):
    ...
    plugin_name: str = Field(
        ...
        min_length=3,
        max_length=32,
        description="插件名称，必须与令牌申请时一致。",
    )
    plugin_developer: str = Field(
        alias="pluginDeveloper",
        min_length=3,
        max_length=32,
        description="插件开发者名称，必须与令牌申请时一致。",
    )
    authentication_token: str = Field(
        alias="authenticationToken",
        min_length=1,
        max_length=64,
        description="已获取到的认证令牌。",
    )
