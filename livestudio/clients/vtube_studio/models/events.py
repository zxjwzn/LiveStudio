...
class TestEventConfig(EventSubscriptionConfig):
    ...
    test_message_for_event: str | None = Field(
        default=None,
        alias="testMessageForEvent",
        max_length=32,
        description="测试事件中回显的文本",
    )
...
class HotkeyTriggeredEventConfig(EventSubscriptionConfig):
    ...
    only_for_action: str | None = Field(
        default=None,
        alias="onlyForAction",
        description="按热键动作过滤",
    )
    ignore_hotkeys_triggered_by_api: bool = Field(
        default=False,
        alias="ignoreHotkeysTriggeredByAPI",
        description="是否忽略由 API 触发的热键",
    )
...
class ModelAnimationEventConfig(EventSubscriptionConfig):
    ...
    ignore_live2d_items: bool = Field(
        default=False,
        alias="ignoreLive2DItems",
        description="是否忽略 Live2D 道具动画",
    )
    ignore_idle_animations: bool = Field(
        default=False,
        alias="ignoreIdleAnimations",
        description="是否忽略待机动画",
    )
...
class ItemEventConfig(EventSubscriptionConfig):
    ...
    item_instance_ids: list[str] = Field(
        default_factory=list,
        alias="itemInstanceIDs",
        description="按实例 ID 过滤",
    )
    item_file_names: list[str] = Field(
        default_factory=list,
        alias="itemFileNames",
        description="按文件名包含过滤",
    )
...
class ModelClickedEventConfig(EventSubscriptionConfig):
    ...
    only_clicks_on_model: bool = Field(
        default=True,
        alias="onlyClicksOnModel",
        description="是否仅在点击模型时触发",
    )
...
class EventSubscriptionRequestData(VTSEmptyData):
    ...
    event_name: str | None = Field(
        default=None,
        alias="eventName",
        description="事件名称；取消全部订阅时可为空",
    )
    subscribe: bool = Field(description="为 true 时订阅，为 false 时取消订阅")
    config: EventSubscriptionConfig = Field(
        default_factory=EventSubscriptionConfig,
        description="事件订阅配置",
    )
...
class EventSubscriptionRequest(VTSRequestEnvelope[EventSubscriptionRequestData]):
    ...
    message_type: str = Field(
        default="EventSubscriptionRequest",
        alias="messageType",
        description="订阅或退订事件",
    )
...
class EventSubscriptionResponseData(VTSEmptyData):
    ...
    subscribed_event_count: int = Field(
        alias="subscribedEventCount",
        description="当前会话已订阅事件数量",
    )
    subscribed_events: list[str] = Field(
        alias="subscribedEvents",
        description="当前会话已订阅事件名称列表",
    )
...
class TestEventData(VTSEmptyData):
    your_test_message: str = Field(
        alias="yourTestMessage",
        description="订阅时配置的测试文本",
    )
...
class ModelLoadedEventData(VTSEmptyData):
    model_loaded: bool = Field(
        alias="modelLoaded",
        description="模型是否已加载，false 表示已卸载",
    )
...
class TrackingStatusChangedEventData(VTSEmptyData):
    ...
    right_hand_found: bool = Field(
        alias="rightHandFound",
        description="是否检测到右手",
    )
...
class ModelConfigChangedEventData(VTSEmptyData):
    ...
    model_name: str = Field(alias="modelName", description="模型名称")
    hotkey_config_changed: bool = Field(
        alias="hotkeyConfigChanged",
        description="是否为热键配置变更",
    )
...
class ModelMovedEventData(VTSEmptyData):
    model_id: str = Field(alias="modelID", description="模型 ID")
    model_name: str = Field(alias="modelName", description="模型名称")
    model_position: ModelPosition = Field(
        alias="modelPosition",
        description="模型当前位置、旋转与缩放",
    )
...
class ModelOutlineEventData(VTSEmptyData):
    model_name: str = Field(alias="modelName", description="模型名称")
    model_id: str = Field(alias="modelID", description="模型 ID")
    convex_hull: list[Vector2] = Field(
        alias="convexHull",
        min_length=3,
        description="模型近似凸包点集",
    )
    convex_hull_center: Vector2 = Field(
        alias="convexHullCenter",
        description="凸包中心点",
    )
    window_size: WindowSize = Field(
        alias="windowSize",
        description="当前 VTS 窗口尺寸",
    )
...
class HotkeyTriggeredEventData(VTSEmptyData):
    ...
    hotkey_triggered_by_api: bool = Field(
        alias="hotkeyTriggeredByAPI",
        description="是否由 API 触发",
    )
    model_id: str = Field(alias="modelID", description="关联模型 ID")
    model_name: str = Field(alias="modelName", description="关联模型名称")
    is_live2d_item: bool = Field(
        alias="isLive2DItem",
        description="是否为 Live2D 道具热键",
    )


class ModelAnimationEventData(VTSEmptyData):
    animation_event_type: str = Field(
        alias="animationEventType",
        description="事件类型",
    )
    animation_event_time: float = Field(
        alias="animationEventTime",
        description="事件在动画中的时间点",
    )
    animation_event_data: str = Field(
        alias="animationEventData",
        description="自定义事件文本或保留关键字",
    )
    animation_name: str = Field(alias="animationName", description="动画文件名")
    animation_length: float = Field(
        alias="animationLength",
        description="动画总长度，秒",
    )
    is_idle_animation: bool = Field(
        alias="isIdleAnimation",
        description="是否是待机动画",
    )
    model_id: str = Field(alias="modelID", description="模型 ID")
    model_name: str = Field(alias="modelName", description="模型名称")
...
class ModelClickedEventData(VTSEmptyData):
    ...
    loaded_model_id: str | None = Field(
        default=None,
        alias="loadedModelID",
        description="当前加载模型 ID",
    )
    loaded_model_name: str | None = Field(
        default=None,
        alias="loadedModelName",
        description="当前加载模型名称",
    )
    model_was_clicked: bool = Field(
        alias="modelWasClicked",
        description="本次点击是否命中模型",
    )
    ...
    clicked_art_mesh_count: int = Field(
        alias="clickedArtMeshCount",
        description="命中的 ArtMesh 数量",
    )
    art_mesh_hits: list[ArtMeshHit] = Field(
        alias="artMeshHits",
        description="所有命中的 ArtMesh 详情",
    )


class PostProcessingEventData(VTSEmptyData):
    current_on_state: bool = Field(
        alias="currentOnState",
        description="后处理系统是否开启",
    )
    current_preset: str = Field(alias="currentPreset", description="当前预设名称")


class Live2DCubismEditorConnectedEventData(VTSEmptyData):
    trying_to_connect: bool = Field(
        alias="tryingToConnect",
        description="VTS 现在是不是在尝试连接 Cubism Editor",
    )
    connected: bool = Field(description="是否已完成连接与鉴权")
    should_send_parameters: bool = Field(
        alias="shouldSendParameters",
        description="是否开启参数直传",
    )
...
class Live2DCubismEditorConnectedEvent(
    VTSResponseEnvelope[Live2DCubismEditorConnectedEventData],
):
    """Cubism Editor 连接状态事件的外壳"""
