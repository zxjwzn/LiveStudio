...
class HotkeysInCurrentModelRequestData(VTSEmptyData):
    ...
    model_id: str | None = Field(
        default=None,
        alias="modelID",
        description="可选模型 ID；为空时使用当前模型",
    )
    live2d_item_file_name: str | None = Field(
        default=None,
        alias="live2DItemFileName",
        description="可选 Live2D 道具文件名；当提供 model_id 时会被忽略",
    )


class HotkeysInCurrentModelRequest(
    VTSRequestEnvelope[HotkeysInCurrentModelRequestData],
):
    """查询热键列表请求"""

    message_type: str = Field(
        default="HotkeysInCurrentModelRequest",
        alias="messageType",
        description="请求模型或道具的热键列表",
    )
...
class HotkeysInCurrentModelResponseData(VTSEmptyData):
    ...
    model_loaded: bool = Field(
        alias="modelLoaded",
        description="目标模型当前是否加载",
    )
    model_name: str = Field(alias="modelName", description="模型名称")
    model_id: str = Field(alias="modelID", description="模型 ID")
    available_hotkeys: list[Hotkey] = Field(
        alias="availableHotkeys",
        description="可用热键列表",
    )


class HotkeysInCurrentModelResponse(
    VTSResponseEnvelope[HotkeysInCurrentModelResponseData],
):
    """热键列表响应信封"""
...
class HotkeyTriggerRequest(VTSRequestEnvelope[HotkeyTriggerRequestData]):
    ...
    message_type: str = Field(
        default="HotkeyTriggerRequest",
        alias="messageType",
        description="请求执行一个热键",
    )
...
class ExpressionStateRequestData(VTSEmptyData):
    ...
    expression_file: str | None = Field(
        default=None,
        alias="expressionFile",
        description="可选表达式文件名；为空则返回全部表达式",
    )
...
class ExpressionStateRequest(VTSRequestEnvelope[ExpressionStateRequestData]):
    ...
    message_type: str = Field(
        default="ExpressionStateRequest",
        alias="messageType",
        description="请求表达式激活状态",
    )
...
class ExpressionActivationRequestData(VTSEmptyData):
    ...
    expression_file: str = Field(
        alias="expressionFile",
        description="目标表达式文件名，必须以 `.exp3.json` 结尾",
    )
    fade_time: float | None = Field(
        default=None,
        alias="fadeTime",
        ge=0,
        le=2,
        description="淡入要多久，单位秒，默认 0.25",
    )
    active: bool = Field(description="`true` 打开表情，`false` 关闭表情")
...
class ExpressionActivationRequest(VTSRequestEnvelope[ExpressionActivationRequestData]):
    ...
    message_type: str = Field(
        default="ExpressionActivationRequest",
        alias="messageType",
        description="请求直接激活或关闭表达式",
    )
