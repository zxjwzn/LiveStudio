...
class VTSRequestEnvelope(VTSBaseModel, Generic[RequestDataT]):
    ...
    api_name: str = Field(
        ...
        alias="apiName",
        description="固定的接口名称，所有请求都要写成 `VTubeStudioPublicAPI`",
    )
    ...
    request_id: str = Field(
        ...
        min_length=1,
        max_length=64,
        description="请求的唯一标识，建议每次都带上，方便找到对应返回和查问题",
    )
    message_type: str = Field(
        alias="messageType",
        description="请求消息类型，比如 `StatisticsRequest`",
    )
    data: RequestDataT = Field(
        description="请求里带的数据，就算有些接口用不上，发空对象一般也没事",
    )
...
class VTSAPIErrorEnvelope(VTSBaseModel):
    ...
    message_type: str = Field(alias="messageType", description="固定写成接口错误")
    request_id: str = Field(alias="requestID", description="与请求对应的唯一标识")
