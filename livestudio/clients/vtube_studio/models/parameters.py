...
class InputParameterListRequest(VTSRequestEnvelope[VTSEmptyData]):
    ...
    message_type: str = Field(
        default="InputParameterListRequest",
        alias="messageType",
        description="请求当前可用的输入参数列表。",
    )
...
class InputParameterListResponseData(VTSEmptyData):
    ...
    custom_parameters: list[ParameterInfo] = Field(
        alias="customParameters",
        description="全部自定义参数。",
    )
    default_parameters: list[ParameterInfo] = Field(
        alias="defaultParameters",
        description="全部默认输入参数。",
    )
...
class ParameterValueRequest(VTSRequestEnvelope[ParameterValueRequestData]):
    ...
    message_type: str = Field(
        default="ParameterValueRequest",
        alias="messageType",
        description="请求单个参数的当前值。",
    )
...
class Live2DParameterListRequest(VTSRequestEnvelope[VTSEmptyData]):
    ...
    message_type: str = Field(
        default="Live2DParameterListRequest",
        alias="messageType",
        description="请求当前模型所有 Live2D 参数。",
    )
    data: VTSEmptyData = Field(
        default_factory=VTSEmptyData,
        description="该请求无需额外参数。",
...
class ParameterCreationRequestData(VTSEmptyData):
    ...
    parameter_name: str = Field(
        ...
        min_length=4,
        max_length=32,
        description="参数名称，仅允许字母数字且不能包含空格。",
    )
    explanation: str | None = Field(
        default=None,
        max_length=255,
        description="可选说明文本，指导用户如何使用该参数。",
    )
...
class ParameterCreationRequest(VTSRequestEnvelope[ParameterCreationRequestData]):
    ...
    message_type: str = Field(
        default="ParameterCreationRequest",
        alias="messageType",
        description="请求创建或覆盖同名自定义参数。",
    )
...
class ParameterDeletionRequest(VTSRequestEnvelope[ParameterDeletionRequestData]):
    ...
    message_type: str = Field(
        default="ParameterDeletionRequest",
        alias="messageType",
        description="请求删除自定义输入参数。",
    )
...
class InjectParameterDataRequestData(VTSEmptyData):
    ...
    face_found: bool | None = Field(
        default=None,
        alias="faceFound",
        description="可选：显式告知是否检测到人脸。",
    )
    mode: Literal["set", "add"] = Field(
        default="set",
        description="注入模式：`set` 覆盖、`add` 累加。",
    )
    parameter_values: list[InjectParameterValue] = Field(
        alias="parameterValues",
        min_length=1,
        description="待注入的参数值列表。",
    )
...
class InjectParameterDataRequest(VTSRequestEnvelope[InjectParameterDataRequestData]):
    ...
    message_type: str = Field(
        default="InjectParameterDataRequest",
        alias="messageType",
        description="请求向默认或自定义参数写入数据。",
    )
...
class GetCurrentModelPhysicsRequest(VTSRequestEnvelope[VTSEmptyData]):
    ...
    message_type: str = Field(
        default="GetCurrentModelPhysicsRequest",
        alias="messageType",
        description="请求当前模型的物理配置。",
    )
    data: VTSEmptyData = Field(
        default_factory=VTSEmptyData,
        description="该请求无需额外参数。",
...
class GetCurrentModelPhysicsResponseData(VTSEmptyData):
    ...
    model_name: str = Field(alias="modelName", description="模型名称。")
    model_id: str = Field(alias="modelID", description="模型 ID。")
    model_has_physics: bool = Field(
        alias="modelHasPhysics",
        description="模型是否存在可用物理配置。",
    )
    physics_switched_on: bool = Field(
        alias="physicsSwitchedOn",
        description="用户是否启用物理。",
    )
    using_legacy_physics: bool = Field(
        alias="usingLegacyPhysics",
        description="是否启用旧版物理。",
    )
    physics_fps_setting: int = Field(
        alias="physicsFPSSetting",
        description="物理帧率设置，-1 表示跟随应用帧率。",
    )
    base_strength: int = Field(alias="baseStrength", description="基础物理强度。")
    base_wind: int = Field(alias="baseWind", description="基础风力。")
    api_physics_override_active: bool = Field(
        alias="apiPhysicsOverrideActive",
        description="是否有插件正在覆盖物理配置。",
    )
    api_physics_override_plugin_name: str = Field(
        alias="apiPhysicsOverridePluginName",
        description="当前占用物理控制权的插件名称。",
    )
    physics_groups: list[PhysicsGroup] = Field(
        alias="physicsGroups",
        description="物理分组配置列表。",
    )


class GetCurrentModelPhysicsResponse(
    VTSResponseEnvelope[GetCurrentModelPhysicsResponseData],
):
    """当前模型物理配置响应信封。"""
...
class SetCurrentModelPhysicsRequestData(VTSEmptyData):
    ...
    strength_overrides: list[PhysicsOverride] = Field(
        default_factory=list,
        alias="strengthOverrides",
        description="物理强度覆盖项。",
    )
    wind_overrides: list[PhysicsOverride] = Field(
        default_factory=list,
        alias="windOverrides",
        description="风力覆盖项。",
    )
...
class SetCurrentModelPhysicsRequest(
    VTSRequestEnvelope[SetCurrentModelPhysicsRequestData],
):
    """覆盖当前模型物理设置请求。"""

    message_type: str = Field(
        default="SetCurrentModelPhysicsRequest",
        alias="messageType",
        description="请求暂时覆盖物理强度/风力设置。",
    )
