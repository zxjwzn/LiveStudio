...
from .defaults import default_plugin_parameters
...
class VTubeStudio(PlatformService):
    ...
    async def _ensure_plugin_parameters(self) -> None:
        ...
        for spec in default_plugin_parameters():
            try:
    ...
    async def _refresh_parameter_specs(
        ...
        """从 VTube Studio 当前可注入输入参数同步平台参数范围。"""
        ...
        await self.model_config_manager.save()
    ...
    async def apply_native_expressions(
        ...
        *,
        ...
        if self._expression_adapter is None:
            return
        await self._expression_adapter.apply(
            list(triggers), self.client, fade_time=fade_time, scope=scope
        )
